...
class Model(object):
    ...
    def inference(self, input=None):
        ...
        with tf.name_scope('Conv1'): 
            ...
            h_conv1 = slim.conv2d(images_re, 32, [5, 5], scope='conv1')
            print_tensor_shape(h_conv1, 'Conv1 shape') 

        # Pooling layer
        with tf.name_scope('Pool1'):

            h_pool1 = slim.max_pool2d(h_conv1, [2, 2], 2, scope='pool1')
            print_tensor_shape(h_pool1, 'Pool1 shape') 

        # Convolution layer
        with tf.name_scope('Conv2'):

            h_conv2 = slim.conv2d(net, 64, [5, 5], scope='conv2d')
            print_tensor_shape(h_conv2, 'Conv2 shape')
        
        # Pooling layer
        with tf.name_scope('Pool2'):

            h_pool2 = slim.max_pool2d(h_conv2, [2, 2], 2, scope='pool2')

        
        net = slim.flatten(h_pool2)
